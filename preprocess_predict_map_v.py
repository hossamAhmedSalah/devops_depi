...
def prepare_payload(image_array, mode):
    ...
    if mode == 'append':
        ...
        raise ValueError("Invalid mode. Please use 'append' or 'overwrite'.")

    with open('request_payload.json', 'w') as f:
        json.dump(request_payload, f)

# Send request and get prediction
def send_prediction(url, payload_file):
...
# Map prediction to human-readable class
def map_prediction(predictions, label_map):
    readable_predictions = []
    for prediction in predictions:
        predicted_class_index = np.argmax(prediction)
        predicted_class_name = label_map.get(predicted_class_index, "Unknown")
        readable_predictions.append(predicted_class_name)
...
def main(image_path, mode, label_map, model_version=None):
    ...
    predictions = send_prediction(url, 'request_payload.json')
    readable_predictions = map_prediction(predictions, label_map)
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("Usage: python script.py <image_path> <mode> [<model_version>]")
        sys.exit(1)
    ...
    model_version = sys.argv[3] if len(sys.argv) == 4 else None
    ...
    main(image_path, mode, label_map, model_version)
